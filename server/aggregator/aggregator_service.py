...
def apply_scoring_logic(shows_data: List[Dict]) -> List[Dict]:
    ...
    for item in temp_scored_shows:
        ...
        market_ask = market.get("yes_ask", 100)
        market_bid = market.get("yes_bid", 0)
        ...
        # Sell/Short Signal
            ...
            recommendation = "BUY NO" # Replaced "SELL YES" with "BUY NO"
...
def recalculate_scores() -> Dict:
    ...
    if not latest or "shows" not in latest:
        return {"error": "No existing analysis found."}
    
    print("Recalculating scores based on cached data...")
    new_shows_list = apply_scoring_logic(latest["shows"])
